...
const STATUS_LABELS = {
  AUTO_APPROVED: "Otomatik geçti",
  PENDING_HUMAN: "Onay bekliyor",
  APPROVED:      "Onaylandı",
  REJECTED:      "Reddedildi",
  BLOCKED:       "Durduruldu",
};
  ...
  return (
    <div className="screen-gecmis">
      <div className="screen-header">
        <h2 className="screen-title">Karar Geçmişi</h2>
        <span className="header-count">{decisions.length} karar</span>
      </div>
      ...
      <div className="gecmis-list">
        {filtered.length === 0 ? (
          <div className="empty-state">
            {decisions.length === 0
              ? "Henüz karar yok. Ana ekrandan analiz başlat."
              : "Eşleşen karar bulunamadı."}
          </div>
        ) : (
          filtered.map((d) => (
            <div key={d.id} className="gecmis-row">
              <span className="gecmis-icon">
              ...
              <div className="gecmis-meta">
                <span className="gecmis-score">Risk: {d.riskScore}/10</span>
                <span className="gecmis-status">
                  {STATUS_LABELS[d.status] || d.status}
                </span>
                <span className="gecmis-time">
                  {d.timestamp ? timeAgo(d.timestamp) : d.ago || ""}
                </span>
              </div>
            </div>
          ))
